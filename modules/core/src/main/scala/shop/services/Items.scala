...
object Items {
  def make[F[_]: Concurrent: GenUUID](
    ...
    new Items[F] {
      ...
      def findAll: F[List[Item]] = postgres.use(_.execute(selectAll))

      def findBy(brand: BrandName): F[List[Item]] = postgres.use { session: Session[F] =>
        session.prepare(selectByBrand).use { ps =>
          ps.stream(brand, 1024).compile.toList
        }
      }

      def findById(itemId: ItemId): F[Option[Item]] = postgres.use { session: Session[F] =>
      ...
      def update(item: UpdateItem): F[Unit] = postgres.use { session: Session[F] =>
